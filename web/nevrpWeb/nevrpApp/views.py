...
def get_node_loation():
    data = pd.read_csv(os.path.join(os.getcwd(), '..', '..', 'data', 'node2.tsv'), sep='\t')
    res = {}
...
def search_vehicle(request):
    ...
    for i in range(len(vehicle_route)):
        ...
        for j in range(len(route_data['start_node'])):
            ...
            concept.append({'routes_number': r_n, 'start_node': address_map[route_data['start_node'][j]] + f" ({str(route_data['start_node'][j])})", 'end_node': address_map[route_data['end_node'][j]] + f" ({str(route_data['end_node'][j])})", 'type': route_data['type'][j], 'phase': route_data['phase'][j], 'view_in_maps': gmap_header+gmap_body})
    # print(concept)
...
#     global __model
#     with open("Bangalore-house-price-prediction/server/artifacts/columns.json", "r") as f:
#         __data_columns = json.load(f)['data columns']
#         __locations = __data_columns[3:]
#     with open('Bangalore-house-price-prediction/server/artifacts/bengaluru_home_prices_model.pickle', "rb") as f:
#         __model = pickle.load(f)
#     print("loading saved artifacts...done")

def select_algo(request):
    return render(request, 'nevrpApp/select_algo.html', {})

def run_algo(request):
    if request.method == 'POST':
        algo= request.POST["algo"]
        linkage = request.POST["linkage"]
        with open('algo_log.txt', 'w') as f: 
            f.write(algo)
            f.write('\n')
            f.write(linkage)
        ...
        print(dict)
        return render(request, 'nevrpApp/validate.html', dict)   