...
def port_correlation2(filename, node_fname):
    ...
    for i in range(len(node_data)): 
        ...
        for j in range(len(node_data)):
            ...
            if line_i['type'] in ['GD1', 'GD2'] and line_j['type'] in ['GD1', 'GD2']: flag = 1
            elif line_i['district_code'] == line_j['district_code']:
                if line_i['type'] == 'GD3' or line_j['type'] == 'GD3': flag = 1
            if flag == 1:
        ...
        with open(f"data/correlations2.{line_i['code']}.tsv", 'a') as f:
            save_data.to_csv(f, sep='\t', index=False, header=False)
    ...
    save_data = pd.DataFrame(data = {
                                    ...
                                    'to_node_code': to_node_code,
                                    'to_node_type': to_node_type})
    with open(f'data/correlations2.transship.tsv', 'a') as f:
        save_data.to_csv(f, sep='\t', index=False, header=False)

    # save_data.to_csv('correlations2.2.tsv', sep='\t', index=False)
...
if __name__ == '__main__':
    ...
    port_correlation2('a', r'data/node2.tsv')
